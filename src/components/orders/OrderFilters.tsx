...
export const OrderFilters = ({ orders, onRetryPayment, onCancelOrder, activeTab, onTabChange }: OrderFiltersProps) => {
  return (
    <Tabs value={activeTab} onValueChange={onTabChange} className="w-full">
      <TabsList className="grid w-full grid-cols-6 mb-4 md:mb-6 h-8 md:h-10">
        ...
        <TabsTrigger value="cancelled" className="text-xs md:text-sm px-1 md:px-3">Batal</TabsTrigger>
      </TabsList>

      <TabsContent value={activeTab}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
          {orders.map((order) => (
            <OrderCard key={order.id} order={order} onRetryPayment={onRetryPayment} onCancelOrder={onCancelOrder} />
          ))}
        </div>
      </TabsContent>
    </Tabs>
